...
import org.usfirst.frc.team5976.robot.GeneratedProfile;
...
public class MotionProfileController {
	 ...
	 */
	public MotionProfileController(CANTalon talon, int multiplier) {
		_talon = talon;
		/*
	...
	public void control() {
		...
		if (_talon.getControlMode() != TalonControlMode.MotionProfile) {
			...
			_state = 0;
			_loopTimeout = -1;
			System.out.println("Unexpectedly exited motion profile control mode");
		} else {
			/*
	...
	private void startFilling() {
		...
		startFilling(GeneratedProfile.Points, GeneratedProfile.kNumPoints);
	}
	...
	private void startFilling(double[][] profile, int totalCnt) {
		...
		for (int i = 0; i < totalCnt; ++i) {
			...
			point.position = profile[i][0] * _multiplier;
			point.velocity = profile[i][1] * _multiplier;
			point.timeDurMs = GeneratedProfile.INTERVAL;
	...
	void startMotionProfile() {
		System.out.println("Started motion profile for " + _talon.getDeviceID());
		_bStart = true;
	}
